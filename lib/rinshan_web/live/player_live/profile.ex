defmodule RinshanWeb.PlayerLive.Profile do
  ...
  def render(assigns) do
    ...
    <div class="space-y-12 divide-y">
      <div>
        <.simple_form for={@player_form} id="player_form" phx-submit="upsert_player">
          <.input field={@player_form[:name]} label="Display Name" required />
          <.input field={@player_form[:discord_id]} label="Discord ID" />
          <:actions>
  ...
  def handle_event("upsert_player", params, socket) do
    ...
    case Players.update_user_player(
           user,
           put_in(params, ["player", "id"], user.player && user.player.id)
         ) do
      {:ok, %{player: player}} ->
        player_form =
          player
